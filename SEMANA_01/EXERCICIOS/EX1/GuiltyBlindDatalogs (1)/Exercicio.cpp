...
int calcular(float medida, float min, float max){ 
...
int Leiturasensor(){
  int valor = 0 
  cout << "Digite o valor do sensor desejado:";  //stream de saída - lembrar* 
  cin >> valor;  //stream de entrada -lembrar*
  return valor; 
...
bool mapeando({
char resposta; 
cout << "Deseja continuar o mapeamento? (sim/não):" //stream de saída
cinn >> resposta //stream de entrada

if (resposta == "sim") || (resposta == "Sim"){ //resposta alternativa caso haja em maíuscula
      return true; 
  
  } else if (resposta == "não") || (resposta == "Não"){ //resposta alternativa caso haja em maíscula
      return false;
  }
}
